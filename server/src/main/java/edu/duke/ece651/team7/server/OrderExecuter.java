...
public class OrderExecuter {
     ...
     */
    public OrderExecuter(Set<Territory> terris){
    ...
    public String doOneMove(MoveOrder o){
        //need rule checker
        o.getSrc().decreaseUnitsBy(o.getUnits());
        o.getDest().increaseUnitsBy(o.getUnits());
        return null;
    }
    ...
    public String pushCombat(AttackOrder o){
        ...
        for(Order order: attackOrderPool.get(o.getDest())){
            if (order.getPlayer().equals(o.getPlayer())){
                order.increaseUnitsBy(o.getUnits());
                flag = 1;
            }
        }
        if(flag == 0){
    ...
    public boolean doOneUnitCombat(CombatOrder defenser, CombatOrder attacker){
        ...
        if(attack.throwDice()> defense.throwDice()){
            defenser.decreaseUnitsBy(1);
            return true;
        }else{
            attacker.decreaseUnitsBy(1);
            return false;
        }
    }

    /**
     * resolve all combats results saved in attackOrderPool
     */
